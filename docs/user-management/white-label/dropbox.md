# Dropbox User Management - White-Label Approach

Manage Dropbox users in white-label connectors.

## User Management API Route

Create a file at `app/api/manage-dropbox-user/route.ts`:
...
const updateUserFiles = async (connectorId: string, userId: string, newSelectedFiles: Record<string, any>, refreshToken: string) => {
  try {
    const response = await fetch("/api/manage-dropbox-user", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export default function DropboxUserManagement() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">Dropbox User Management</h2>
