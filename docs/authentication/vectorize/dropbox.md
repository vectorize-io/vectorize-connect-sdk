# Dropbox Authentication - Vectorize Approach

Authenticate users with Dropbox using Vectorize's managed OAuth flow.
...
Create an API endpoint to generate one-time tokens for Dropbox authentication:

```typescript
// app/api/get-dropbox-token/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error generating Dropbox token:', error);
    return NextResponse.json({ 
...
export default function DropboxVectorizeConnector() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">Dropbox Vectorize Connection</h2>
